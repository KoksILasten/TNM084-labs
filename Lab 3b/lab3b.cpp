...
#define bumpWidth 2.0
			...
			{
				y += amplitude * (snoise2(frequency * x, frequency * z));
				// std::cout << y << " " <<h <<"\n";
				frequency *= lacunarity;
				amplitude *= gain;
			}
			//std::cout << snoise2(frequency * x, frequency * z);
			vertices[ix] = vec3(x * kPolySize, y, z * kPolySize);
			texCoords[ix] = vec2(x, z);

			// cross()
			// normals[ix] = normalize(vec3(x,y,z));
	...
	for (int x = 0; x < kTerrainSize; x++)
		...
		{
			// cross
			vec3 p1 = vertices[(z + 1) * kTerrainSize + (x + 1)];
			vec3 p2 = vertices[(z + 1) * kTerrainSize + (x - 1)];
			vec3 p3 = vertices[(z - 1) * kTerrainSize + (x + 1)];
			vec3 p4 = vertices[(z - 1) * kTerrainSize + (x - 1)];

			vec3 line1 = p4 - p1;
			vec3 line2 = p2 - p3;
			vec3 skibidnormal = cross(line1, line2);
			skibidnormal = normalize(skibidnormal);
            if(skibidnormal.y < 0){
               skibidnormal = vec3(0,1,0);
            }
			normals[z * kTerrainSize + x] = skibidnormal;
			//std::cout << skibidnormal.y << std::endl;
		}
}
